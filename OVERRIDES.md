...
| `config/settings_schema.json` | 02-01, 02-02, 03-04 | 02-01: grupo `t:general.colors` extendido con el header `t:settings.colors.brand` y `color_primary`. 02-02: se completó la superficie de marca — las diez familias de color (seis primarias + cuatro de apoyo) como settings `color` con el hex verbatim del Brand Book de default; se reemplazó `background_color` (`#FFFFFF`, viola D-03) y `foreground_color` por `color_bg` / `color_text`; el `font_picker` `type_primary_font` pasó a dos `select` (`font_heading` / `font_body`, D-18); `input_corner_radius` `max`/`default` bajados a 2 para que el editor no pueda romper el sistema plano (D-10). `theme_info` sigue siendo el elemento 0. 03-04: nuevo grupo `t:general.whatsapp` con el setting de texto libre `whatsapp_number` (sin default — un número inventado es peor que un botón vacío, D-15), usado únicamente como target de link. `theme_info` sigue siendo el elemento 0. |
...
| `ALLOWLIST.md` | 01-04, 02-01, 02-03, 02-04, 03-01, 03-04 | 03-01: filas nuevas en `## Renderiza` para `snippets/analytics-hooks.liquid` y `assets/events.js`; la fila de `## Nunca agregar` sobre JS pasa de "prohibido en Fase 1" a "solo vía `JS_ASSET_ALLOWLIST`"; la desviación registrada "bus de eventos DOM + helpers a11y → Fase 3" se marca entregada. 03-04: fila de `sections/footer.liquid` reescrita para el menú legal + WhatsApp + newsletter + pagos; filas nuevas para `assets/icon-whatsapp.svg` y el botón flotante; puntero a `docs/RUNBOOK-STAGING.md`. |
| `locales/es.default.schema.json` (era `en.default.schema.json`) | 02-01, 02-02, 02-04, 03-03, 03-04 | 02-01: se quitó la coma colgante y se agregaron `settings.colors.brand` / `primary` / `primary_info`. 02-02: etiquetas de editor en español para cada clave `t:` nueva del schema — las diez familias de color, `settings.type.heading` / `body`, y los `info` de advertencia (uso restringido del terracota, nunca blanco puro). 02-04: `git mv` de `en.default.schema.json` → `es.default.schema.json` (rename, no borrado — Pitfall 9) y traducción de los namespaces `general` / `labels` / `options` al español; strict JSON, sin BOM, LF. 03-03: nuevo namespace `sections.announcement_bar` — nombre de grupo del editor y las dos etiquetas de setting (mensaje personalizado, link). 03-04: `general.whatsapp`, `settings.whatsapp.number` / `.number_info` (la nota de formato del número) y `labels.show_newsletter`. |
| `locales/es.default.json` (era `en.default.json`) | 02-04, 03-03, 03-04 | 02-04: `git mv` de `en.default.json` → `es.default.json` (rename con historia preservada, no borrado — Pitfall 9). Traducido a voseo rioplatense: cada namespace de inglés preservado (404, blog, cart, customers, collections, gift_card, password, search) más `general` / `products` / `sections` / `templates` / `newsletter`. Nuevo namespace `kinelia` con los seis CTAs aprobados, la promesa raíz y la leyenda legal (D-15). Sin precio placeholder (contenido de metaobject de la Fase 4). `_html` solo en las claves que ya lo llevaban. Strict JSON, sin BOM, LF. 03-03: `sections.announcement_bar.message` y `.link_label` — el mensaje de la franja de anuncio (envío nacional + garantía de 90 días, sin urgencia fabricada, D-16) y el nombre accesible de su link opcional. 03-04: `sections.footer.legal_links_heading` — el título del menú legal del footer. |
...
| `sections/footer.liquid` | 02-02, 03-04 | 02-02: rename en el bloque `{% stylesheet %}`: `var(--color-foreground)` → `var(--color-text)`. Marcador KINELIA. Sin cambio estructural. 03-04: cierra SHELL-02 — el `link_list` legal queda detrás de `menu.links.size > 0` con encabezado del locale y ningún target de página tipeado (D-14); nuevo link de WhatsApp que lee `settings.whatsapp_number` (nunca en el `{% stylesheet %}`, T-03-17), con `rel="noopener noreferrer"` y sin render cuando el setting está vacío; bloque de newsletter tras `show_newsletter` (checkbox, default `false`, D-16); bloque de pagos del starter intacto (D-17); botón flotante `.wa-fab` como hermano de `<footer>`, al nivel `--z-fab` del plan 03-03, con la regla `body.has-sticky-atc` ya escrita para que la Fase 6 solo agregue la clase. |
...
### Cierre de la Fase 3 (plan 03-04)

SHELL-02 queda satisfecho entre 03-03 (header + franja) y 03-04 (footer + FAB). Resumen de
lo que la Fase 3 cambió en el shell del tema y por qué, para quien llegue a este documento
sin haber leído los cuatro planes:

- **El reorden del `<head>`** (03-01) — las tres meta de codificación/compatibilidad/área
  visible suben arriba del bloque de tokens para que la codificación caiga dentro de la
  ventana de bytes que el navegador lee antes de parsear (Pitfall 1).
- **El primer asset de script del tema** (03-01) — `assets/events.js`, admitido vía
  `JS_ASSET_ALLOWLIST`, precedente que las Fases 5/6/11 heredan.
- **La primera section nueva del tema** (03-03) — `sections/announcement-bar.liquid`,
  montada antes del header en `header-group.json`.
- **El primer elemento flotante del tema** (03-04) — el botón `.wa-fab`, hermano de
  `<footer>`, al nivel `--z-fab`; cero JavaScript, cero elevación, cero animación.
- **La desviación de orden de apilamiento** (03-03, arriba) — heredada sin cambios por
  03-04: el FAB vive bajo el nivel de overlay, no por encima del cart drawer.

Ningún archivo del starter se borró en el proceso; el footer se extendió, no se reescribió,
y el linklist legal, el número de WhatsApp y el menú del footer quedan como pasos de
`docs/RUNBOOK-STAGING.md` — un paso de admin que el código no puede ejecutar entra al
runbook en la misma pull request que crea el enganche que lo necesita.
...
| `assets/icon-whatsapp.svg` | 03-04 | Glyph de contacto (D-15) — una burbuja de diálogo con handset trazada, no el logo relleno de otra empresa. Mismas convenciones que `icon-cart.svg`/`icon-account.svg`: viewBox 20×20, sin relleno, trazo en `currentColor` con el token `--icon-stroke-width`. Inlineado por `sections/footer.liquid` en el link del footer y en el botón flotante. |
| `docs/RUNBOOK-STAGING.md` | 03-04 | Los pasos de admin que el código de esta fase no puede ejecutar (docs/RELEASE.md §"Propiedad del contenido", SHELL-02): las cinco páginas stub con su slug exacto, el menú del footer que las enlaza, y el número de WhatsApp en el setting del tema. Cierra el gate de fin de fase de SHELL-02. |
...
- Un paso de admin que el código no puede ejecutar (crear una página, armar un menú, cargar un setting con un valor real) se escribe en `docs/RUNBOOK-STAGING.md` en la misma pull request que crea el enganche que ese paso completa — nunca como una tarea recordada de palabra.
